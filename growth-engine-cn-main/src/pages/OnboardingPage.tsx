...
type FormState = {
  companyName: string; marketFocus: string; annualRevenue: string;
  channelMix: string; adPlatforms: string; attributionMaturity: string;
  skuStructure: string; heroSkuRatio: string; contentCapacity: string;
  creativeRhythm: string; teamStructure: string; decisionCycle: string;
  goal90: string; accessChecklist: string; priorityIssues: string;
};

const initialState: FormState = {
  companyName: '', marketFocus: '', annualRevenue: '',
  channelMix: '', adPlatforms: '', attributionMaturity: '',
  skuStructure: '', heroSkuRatio: '', contentCapacity: '',
  creativeRhythm: '', teamStructure: '', decisionCycle: '',
  goal90: '', accessChecklist: '', priorityIssues: ''
};

type StepConfig = { title: string; fields: Array<{ key: keyof FormState; label: string; hint: string }> };

const steps: StepConfig[] = [
  { title: '企业画像与营收结构', fields: [
...
export function OnboardingPage() {
  ...
  return (
    <div className="shadcn-scope site-shell onboarding-shell">
      <header className="top-bar">
        <div className="logo">客户入舱流程</div>
        <nav>
          <Link to="/" className="nav-cta">← 返回主站</Link>
        </nav>
      </header>

      <main className="onboarding-main">
        {/* Progress header */}
        <section className="section" style={{ padding: '24px 28px' }}>
          <h1 style={{ fontSize: '22px', marginBottom: '8px' }}>签约客户入舱表单</h1>
        ...
        <section className="section">
          {submitted ? (
            <div className="submit-panel">
              ...
              <Link to="/" className="btn-primary">← 返回主页</Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <h2 style={{ marginBottom: '20px' }}>{current.title}</h2>
                ...
                {current.fields.map((field) => (
                  <label key={field.key} className="form-item">
                    ...
                    <textarea
                      ...
                      onChange={(e: ChangeEvent<HTMLTextAreaElement>) => updateField(field.key, e.target.value)}
